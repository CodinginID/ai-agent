...
### Task 1: Endpoint gateway `POST /chat/approve` + `POST /chat/reject` ✅ SELESAI

**Files:**
- Modified: `app/interfaces/chat.py` (tambah `PlanDecisionRequest`, `_stream_approval`, `/approve`, `/reject`)
- Created: `tests/interfaces/test_chat_approval.py` (5 test)

**Status:** Semua 5 test PASS. Implementasi ada di `chat.py:253-318` (setelah `chat_send`).

> ⚠️ **Note:** Implementasi dan test sudah ada, tapi checklist di plan ini belum di-update secara manual ke `[x]`. PR: #77.
